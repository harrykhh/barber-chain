...
module.exports = function(app){
	/* GET home page. */
	app.get('/editReport/:id', function(req, res) {
		var param = {};
		if (req.user){
			...
			storeLog.find({
			...
			}, { 'date': 1, '_id': 0}).sort({'date': 1}).exec(function(err, obj) {
				var param = {};
				if (err){
					console.error("Error Occured!", err);
					req.session['submiterror'] = true;
					req.session['user'] = req.user;
					return res.redirect('/editReport/' + req.params.id);
				}
				param.title = "Edit Report";
				...
				param.user = req.user;
				if (obj) {
					param.reportDate = Object.keys(obj).map(function(k) { return moment.utc(obj[k].date).format('MM-DD-YYYY') });
					return res.render('editReport', param);
				} else {
	...
	app.post('/editReport/:id', function(req, res, next) {
					var ssales = [];
